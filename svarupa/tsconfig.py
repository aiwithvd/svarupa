...
__all__ = ["load_aliases", "strip_jsonc"]
...
def _read(path: Path) -> dict[str, object] | None:
    try:
        ...
        return None
    try:
        loaded: object = json.loads(_TRAILING_COMMA.sub(r"\1", strip_jsonc(raw)))
    except json.JSONDecodeError:
        return None
    return cast("dict[str, object]", loaded) if isinstance(loaded, dict) else None


def load_aliases(root: Path) -> tuple[tuple[str, str], ...]:
    """Collect `paths` aliases from every reachable tsconfig.

    Targets are returned repo-relative, since an alias declared in
    `packages/web/tsconfig.json` is relative to that file, not to the repo.
    """
    ...
    while queue and len(seen) < _MAX_CONFIGS:
        ...
        base_str = "" if not isinstance(base, str) or base in (".", "./") else base.strip("./")
        ...
        for key, value in cast("dict[str, object]", raw_paths).items():
            ...
            if not isinstance(first, str):
                continue
            target_rel = first.rstrip("/*").lstrip("./")
            if base_str:
                target_rel = f"{base_str}/{target_rel}".strip("/")
            if prefix:
                target_rel = f"{prefix}/{target_rel}".strip("/")
            aliases.setdefault(str(key).rstrip("/*"), target_rel)
