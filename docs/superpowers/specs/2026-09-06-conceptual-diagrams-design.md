...
| **B. Data flow and request flow** | the two new derivers, their drill, the `not drawn` sentence for lifecycle/workflow, computed cards | each stage node cites; a request-flow view for every api module; report states how many modules fell in no stage |
...
| **D. Gates and stories** | four-viewport containment in CI, guided views, legend and card polish, export | the containment gate fails on a planted overflow |
