...
| **A box is named for what it is, never for one of its members** (review #20 M3) | Community boxes wore their anchor's name and id for nineteen reviews, so `src/api -> scripts (16 imports)` cited six lines none of which imported anything under `scripts/`, the demo's "routers" was the whole agent service, and the passport's "also in" jumped from a group to a module as if they were one box. A group has a `group:<anchor>` id (as stable as the anchor, never a module's) and a label that reads as a set: the directory all members share, else the anchor's leaf plus the count of the rest; the passport lists the members. Communities stay presentation (F2) and now look like it |
| **A module stands for every service whose build context holds it** (review #20 M2) | Two services built from `.` each ship the whole tree; mapping every module to one service gave the demo's `api` no database and cited `api`'s file under `agent`. Where the evidence supports several boxes it draws several arrows with the same line, since a wrong edge is worse than a missing one and a missing one is worse than two true ones |
| **A gesture the reader needs is written on the page and has a button** (review #20 M4) | The drill, the interaction the user asked for by name, existed only as an unannounced double-click; a stranger single-clicked, got a card, and never found the second level. The chevron opens it on a click, the passport offers "Open in place", the explore hint names the gestures, and the documents describe the artifact as it behaves |
| **Past the top-box budget, a complete view follows the directory tree** (review #20 M5) | 30 boxes and 76 arrows validated clean and could not be read; 76 identical `imports` labels doubled the ink. Structural arrows are silent and the legend says once what a solid arrow is; module deps beyond the budget show a box per top-level part, the modules of a part one drill down, and every dependency at exactly one level, so nothing is summarized away and the tree is the identity the lockfile already rests on, never a community |
| **Every text on the canvas is an obstacle for every other** (review #20 S5) | Six band-label collisions with route verbs survived every gate because no gate knew where a band label was. Band and region label rectangles are one definition read by the settle, the validator and the renderer |
| **A crash in a dependency is worked around at the call and pinned in a fresh process** (review #20 M1) | py-tree-sitter 0.26 corrupted the heap when a docstring node's points were read, and the build died with exit 138 and no diagnostic; nothing in-process can catch a signal. The reviewer's diagnosis (a dropped `Tree`) was incomplete and the fix that followed it still crashed; the cause was found by bisecting scanner variants in separate processes. Lines come from byte offsets, and the regression test runs the scan in a subprocess over the files that crashed |
| **The passport is a side panel, not an overlay** (review #20 S3) | Archify's card floats over the canvas and covered the guided strip, the title and two boxes at 1440x900 (a click on one landed on the card). The tab makes room while the card is open; nothing a reader may want sits under it |
| **The ids a diagram draws are ids the graph answers for** (review #20 S7) | `get_node api/routers` returned null for the id the passport showed, because directory modules were a bare list in `graph.json`. They are nodes with the evidence their boxes carry, `contains` their files and `imports` each other with weights and lines |
| **No input is a refusal; a repository with no source is a repository** (review #20 C6) | An empty directory scanned to "0 files", exit 0, and an artifact whose only tab was "not drawn", so a typo landing on an empty directory read as success. SVA-D-008 refuses an empty directory; a directory of files none of which is source still gets its artifact and its named absences |
| **An image holds what its Dockerfile copies, not what its context permits** (review #21 N1) | Mapping a module to every service whose build context held it drew `api -> Gemini API` on the demo, cited to a file only the agent image copies (`Dockerfile.api` does `COPY api /app/api`). The compose extractor reads each built service's Dockerfile and records its COPY/ADD sources with their lines; a module stands for the services whose image copies it, the context is the fallback when no Dockerfile can be read, and a store arrow cites the import and the COPY that puts it in the image. Not read: `.dockerignore`, `target:` stages |
| **A card is titled by the label the reader clicked; ids are chips** (review #21 N2) | The passport opened `group:agent/routers` over a box labelled `agent`, the README's own screenshot included: M3's naming leak, one surface later. Every surface that names a box (card title, connection summary, path caption) uses its label; the id is the chip, and a `group:`/`tree:` id says it is a diagram box with no graph node |
| **A scroll measures the layout that exists** (review #21 N3) | The "Open in place" button closed the side panel and scrolled while the tab's padding was still animating, so the drill landed 57px under the header. Nothing animates the layout between a state change and a scroll, and the header's measured height is the scroll margin, so a two-row header is respected too |
| **Navigation is never hidden to keep a height** (review #21 N4) | A scrolling nav with no scrollbar kept the header at 57px by dropping whole tabs at 1280 and 1024. The controls give way first (label, then input width); the header takes a second row before a tab disappears. Visible tabs beat one row |
| **A decision holds at every supported width** (review #21 N5) | "The passport is a side panel, not an overlay" was true at 1440 and 1280 and false at 1100 by a media query. The panel narrows below 1100 and stays a panel |
| **"In a cycle" is membership of a cycle** (review #21 N6) | The band label came from "what Kahn could not drain", which is the cycle plus everything downstream, and named a leaf `schema` module cyclic. Membership is reachability to self; a floor band mixing a cycle with what hangs below it says `cycle inside` |
| **A name chosen for a box follows it everywhere** (review #21 N7, N8) | The group labelled `agent` drilled into `routers internals`, and on svarupa itself a group was named `svarupa` for the directory its members share while another box held the `svarupa` module. The drill, its subtitle and the chapters use the box's label; a shared-directory name is used only when no other box holds a module under it |
| **An exact id is never ambiguous** (review #21 N11) | `get_node agent` returned two candidates because a compose service was labelled `agent` too. Ids are unique by construction, so the text that is an id names one node; only labels shared by several nodes remain an ambiguity |
| **A verb is drawn within reach of one of its boxes, or not at all** (review #21 N16, polish) | `reads/writes` sat on the top lane of a corridor whose two boxes were 1000px apart: attached to its arrow and unreadable as anything. On a multi-segment route the mask must sit within 240px along the route of one end (a straight arrow's midpoint is between its boxes and is exempt); otherwise the verb is dropped and the passport says it. On descovo 347 such verbs went; the arrows stayed |
| **Tooling under a hidden directory is not architecture** (review #22, self-review) | On a Next.js app nine of twelve top-level architecture boxes were `.agent/skills/*/scripts` and `.claude/skills/*/scripts`, agent scaffolding, and the application sat in a second row off the first screen. Files under a top-level hidden directory are the `tooling` role: in the graph, excluded from derivation and the lockfile like tests and vendored code. A root dotfile and a hidden directory deeper down are not tooling by that rule. Lockfile minor 5 -> 6, since fewer facts for the same code is a change in what the build emits |
| **A gesture is verified with the input it names** (review #23 F1) | Three reviews walked the drill with synthetic clicks at recomputed coordinates and never sent a real double-click; the first click opened the side panel and moved the box 154 to 192px, so the second click closed the card. The viewer remembers the last box clicked and a `dblclick` anywhere in the tab within 700ms opens it; the check is a real `mouse.dblclick` in Chromium, and the harness sends the two clicks the way a hand does |
| **An empty file is cited as itself** (review #23 F3) | `_line_count` floored an empty file at one line so that `pkg/__init__.py:1` could be a citation; 259 of them pointed at a line that did not exist, the first in the README's own screenshot. An empty file has zero lines, is cited as `(0, 0)`, which the build accepts only for a zero-line file, renders as the path alone with "(empty file)", links without a line anchor, and follows the module's real lines |
| **The root module is `.` everywhere** (review #23 F7) | The repository root module was `""` in diagrams, `.` in the lockfile: a box with an empty id chip that `get_node` could not name. Every diagram id and graph.json node spells it `.` |
| **A report quotes the artifact it describes** (review #23 F5, F6) | REPORT.md said 132 nodes where graph.json held 184 (the in-memory graph before modules, routes, externals and rationale were added) and counted invisible routing waypoints as boxes. graph.json is built once and the report quotes it; boxes are the boxes drawn |
| **A verb moves toward its box before it is dropped** (review #23 F4) | The 240px rule skipped every long segment, so deploy arrows whose only room was their long segment lost their verb while the legend promised one. The verb slides along the segment toward the nearer box; only a verb that cannot come within reach is dropped, and the legend says so |
| **A remembered gesture is not overwritten by the gesture that completes it** (review #24 N1) | The double-click fix remembered the last box clicked, and the second click of the pair, landing on the ingress box the layout shift had moved under the cursor, replaced it. A pending drillable click survives its 700ms window whatever the completing click lands on |
| **One order for citations, in every surface** (review #24 N3) | The viewer put a module's real lines before its empty files while the diagram JSON and the CLI kept the canonical sort and printed `api/__init__.py:0`; three surfaces disagreed about the same box. `evidence_order` in the model is the one order, and the CLI spells an empty file as the passport does |
| **A stage frame past 12 boxes is capped, most-connected first, remainder stated** (review #25/#26 scale wave) | MAX_STAGE_BOXES = 12, the top-level budget applied one stage down: the 277-domain-module data-flow root was a 3515x27064 canvas and failed geometry outright; capped it is 21 boxes on 1421x1072 and reads. Reachability claims are computed before the cap (a module reachable only through an omitted handler is still reachable), and the omitted count is stated in the subtitle — dropping silently would make the picture claim the stage is smaller than it is |
| **In-place expansions are pre-rendered only for hosts of at most 24 boxes** (review #25/#26 scale wave) | MAX_EXPANSION_HOST_BOXES = 24. Each expansion re-renders its whole host view, so bytes grow with host size times drillable count: the acceptance repo's 161-box, 176-drillable host alone paid ~90MB of a 383MB index.html. Past a couple of dozen boxes the surrounding context an expansion preserves is not readable anyway, so the drill falls back to the child as its own view and the passport button says "Open" rather than "Open in place"; capped at 24 the file is 113MB |
